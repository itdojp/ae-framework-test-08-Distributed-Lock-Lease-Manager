...
- GitHub Actions:
  ...
  - `Artifacts Sync On Workflow Complete`: `CI Basic` / `AE Eval Fast` / `AE Eval Full` の `workflow_run.completed(success)` を契機に起動し、同一 `head_sha` の成功runをまとめて取り込みして `backfill -> index` を実行（`artifacts-writer-main` で直列化、push競合時はrebase retry）
