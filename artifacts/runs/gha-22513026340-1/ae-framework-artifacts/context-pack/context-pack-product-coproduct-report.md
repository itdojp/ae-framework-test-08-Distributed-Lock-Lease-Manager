# Context Pack Product/Coproduct Validation Report

- GeneratedAt: 2026-02-28T04:10:43.625Z
- Status: PASS
- Map: `spec/context-pack/product-coproduct-map.json`
- Schema: `schema/context-pack-product-coproduct.schema.json`
- Context pack files: 4
- Context morphisms: 1
- Product mappings: 1
- Coproduct mappings: 1
- Failure variant coverage: 1/1
- Violations: 0

## Summary
- missing product mappings: 0
- missing coproduct mappings: 0
- input coverage gaps: 0
- variant coverage gaps: 0
- ambiguous DTO keys: 0
- missing evidence paths: 0

## Violations

No violations detected.
